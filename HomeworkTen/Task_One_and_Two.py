...
def get_most_common_symbol():
    ...
    while True:
        try:
            with open(argv[1], 'r', encoding='UTF-8') as a:
                text = a.read().lower().strip().replace(" ", "")
                result1 = working_pony(text)
                result2 = working_pony_2(text)
                return result1, result2
        except IndexError:  # If there is no system argument
            while True:
                answ = input('Parameter was not set, do you want to input it yourself? [y / n]')
                if answ == 'y':
                    argv.append(input('Input filename'))
                    break
                elif answ == 'n':
                    print('exiting...')
                    return
            ...
            while True:
                answ = input('File was not found, do you want to reinput it yourself? [y / n]')
                if answ == 'y':
                    argv[1] = input('Input filename')
                    break
                elif answ == 'n':
                    print('exiting...')
                    return
